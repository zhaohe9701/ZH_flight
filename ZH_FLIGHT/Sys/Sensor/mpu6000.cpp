...
uint8_t Mpu6000::_GetLpfValue(const uint8_t frq)
{
    uint8_t lpf = 0x01;
    switch (frq)
    {
...
float Mpu6000::_GetAccLsb(uint8_t afs)
{
    float lsb = 4096.0f;
    switch (afs)
    {
...
float Mpu6000::_GetGyroLsb(uint8_t afs)
{
    float lsb = 16.4f;
    switch (afs)
    {
...
void Mpu6000::_GetGyroRawData(int16_t *gx, int16_t *gy, int16_t *gz)
{
    uint8_t buf[6];
    _interface->ReadBytes(MPU_GYRO_XOUTH_REG, 6, buf);
    uint16_t gx_raw, gy_raw, gz_raw;
...
void Mpu6000::_GetAccRawData(int16_t *ax, int16_t *ay, int16_t *az)
{
    uint8_t buf[6];
    _interface->ReadBytes(MPU_ACCEL_XOUTH_REG, 6, buf);
    uint16_t ax_raw, ay_raw, az_raw;
